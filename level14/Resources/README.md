В данном таске нет ни файлов, ни запущеных процессов. Поэтому принимаем решение ломать getflag.
...
Получаем следующий листинг и разбираем его:
...
Заметим несколько одинаковых блоков (15 штук - столько же сколько юзеров flag)
```assembly
   0x08048d99 <+1107>:  call   0x8048530 <fputs@plt>
   0x08048d9e <+1112>:  jmp    0x8048e2f <main+1257>
   0x08048da3 <+1117>:  mov    0x804b060,%eax
...
Найдем jump на любой из блоков например *(&main+1150) и снова видим много повторяющихся блоков
```assembly
   0x08048b68 <+546>:   cmp    $0xbc2,%eax
   0x08048b6d <+551>:   je     0x8048d5b <main+1045>
   0x08048b73 <+557>:   cmp    $0xbc2,%eax
...
Юзер flag14 имеет uid 3014 или 0xbc6 (cat /etc/passwd). И видим сравнение с этим числом по офсету 0x08048bb6 <+624> после которого происходит jump на 0x8048de5 <main+1183>

Поэтому чтобы вытащить ключ нам сначала необходимо загрузить программу, а потом сразу перейти на оффсет 0x8048de5 <main+1183>
...
[Inferior 1 (process 8328) exited normally]
```
